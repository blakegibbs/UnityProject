...
bool CheckOriginalSpriteTexture(float2 uv, bool ifZero)
{
    ...
    // Check basic points first
    if (ifZero)
    {
        // Count how many neighbors are fully transparent
        alphaCounter += SampleSpriteTexture(uv + float2(0, +thicknessY)).a == 0 ? 1 : 0;
        alphaCounter += SampleSpriteTexture(uv + float2(0, -thicknessY)).a == 0 ? 1 : 0;
        alphaCounter += SampleSpriteTexture(uv + float2(+thicknessX, 0)).a == 0 ? 1 : 0;
        alphaCounter += SampleSpriteTexture(uv + float2(-thicknessX, 0)).a == 0 ? 1 : 0;
        alphaCounter += SampleSpriteTexture(uv + float2(+thicknessX, +thicknessY)).a == 0 ? 1 : 0;
        alphaCounter += SampleSpriteTexture(uv + float2(-thicknessX, -thicknessY)).a == 0 ? 1 : 0;
        alphaCounter += SampleSpriteTexture(uv + float2(-thicknessX, +thicknessY)).a == 0 ? 1 : 0;
        alphaCounter += SampleSpriteTexture(uv + float2(+thicknessX, -thicknessY)).a == 0 ? 1 : 0;
    ...
    {
        // Check for any neighboring pixels above the alpha threshold
        outline =
            SampleSpriteTexture(uv + float2(0, +thicknessY)).a > alphaThreshold ||
            SampleSpriteTexture(uv + float2(0, -thicknessY)).a > alphaThreshold ||
            SampleSpriteTexture(uv + float2(+thicknessX, 0)).a > alphaThreshold ||
            SampleSpriteTexture(uv + float2(-thicknessX, 0)).a > alphaThreshold ||
            SampleSpriteTexture(uv + float2(+thicknessX, +thicknessY)).a > alphaThreshold ||
            SampleSpriteTexture(uv + float2(-thicknessX, -thicknessY)).a > alphaThreshold ||
            SampleSpriteTexture(uv + float2(-thicknessX, +thicknessY)).a > alphaThreshold ||
            SampleSpriteTexture(uv + float2(+thicknessX, -thicknessY)).a > alphaThreshold;
    }
